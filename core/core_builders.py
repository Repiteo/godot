...
def disabled_classes(target: str, classes: list[str]):
...
def git_info(target: str, hash: str, timestamp: int):
    with methods.generated_wrapper(target) as file:
        file.write(f"""\
...
def encryption_key(target: str, key: bytes):
    with methods.generated_wrapper(target) as file:
        file.write(f"""\
...
def certs(target: str, certs: str, builtin: bool, system_certs: str):
    import zlib
...
def authors(target: str, authors: str):
    SECTIONS = {
...
def donors(target: str, donors: str):
    SECTIONS = {
...
def license(target: str, license: str, copyright: str):
    from typing import TextIO

    class LicenseReader:
        def __init__(self, license_file: TextIO):
